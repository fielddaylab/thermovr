...
namespace ThermoVR.State
{
    ...
    public class ThermoState : MonoBehaviour
    {
        ...
        */
        ...
        private static double STOP_MARGIN = 0.00001f;
        ...
        public void warp_pv_unobtrusive(double p, double v, double t)
        {
            try
            {
                pressure = p;
        ...
        public void add_heat_per_delta_time(double applied_heat, double insulation_coefficient, double delta_time, double p_outside, bool is_internal, double temperature_gradient)
        {
            ...
            bool constant_v = treat_as_constant_v_add_heat(applied_heat, insulation_coefficient, delta_time, p_outside, out double stopV);
        ...
        private void add_heat_constant_p_per_delta_time(double applied_heat, double insulation_coefficient, double delta_time)
        {  // Pressure Constrained -> Insulated && Uninsulated -> delta energy    // time eqtn 6b
            try
            {
        ...
        private void add_heat_constant_v_per_delta_time(double applied_heat, double insulation_coefficient, double delta_time)
        { // Volume Constrained -> Insulated && Uninsulated ->  delta energy     // time eqtn 6a
            try
            {
        ...
        private void add_heat_constant_t_per_delta_time(double applied_heat, double insulation_coefficient, double delta_time, double temperature_gradient)
        {  // Temperature Constrained (0% insulation)   // time eqtn 5
            try
            {
        ...
        public void add_pressure_uninsulated_per_delta_time(double p, double delta_time, double insulation_coefficient, double p_outside, double temperature_gradient, out bool tryIterate, bool heat_transfer_active)
        {
            tryIterate = false;
            ...
            try
            {
                ...
                switch (region)
                {
                    ...
                    case ThermoMath.region_vapor: //vapor region
                        ...
                        { // check for transition into 2-phase from other states
        ...
        private bool blocked_by_stops(double p_outside, out double stopV, bool isHeat, double applied_heat = 0)
        {
            if (region == 0) { stopV = -1; return false; } // liquid volume doesn't change
                ...
                if (within_vstop_buffer)
                {
                    if (volume < compare_v && pressure >= p_outside)
                    {
                        ...
                        if (isHeat)
                        {
                            if (applied_heat > 0)
                            {
                                stopV = compare_v - STOP_MARGIN;
                                return true;
                            }
                            else if (pressure > p_outside)
                            {
                                return true;
                            }
                            else
            ...
            // needs a way to escape from the buffer zone in 1 tick
        ...
        public void add_pressure_insulated_per_delta_time(double p, double delta_time, double p_outside, double temperature_gradient, out bool tryIterate)
        {
            ...
            try
            {
                ...
                if (region == ThermoMath.region_vapor)
                {
                    helper_add_p_insulated_vapor(new_t, new_u, new_v, new_p, insulation_coefficient, delta_time);
                }
            }
            catch (Exception e) { }
        ...
        private void helper_add_p_insulated_vapor(double new_t, double new_u, double new_v, double new_p, double insulation_coefficient, double delta_time)
        {
            /*
        ...
        /// <returns></returns>
        private bool treat_as_constant_v_add_heat(double applied_heat, double insulation_coefficient, double delta_time, double p_outside, out double stopV)
        {
            bool blocked = blocked_by_stops(p_outside, out stopV, true, applied_heat); // first pass
