...
namespace IF97
{
  ...
  static string IF97VERSION = "v2.1.2";
  ...
  RegionResidualElement[] Hresiddata = new RegionResidualElement[] // Residual H for viscosity
  {
    {0, 0,  5.20094e-1},
    {1, 0,  8.50895e-2},
    {2, 0, -1.08374   },
  ...
  RegionIdealElement[] Hidealdata = new RegionIdealElement[] // Ideal H for viscosity
  {
    {0,  1.67752   },
    {1,  2.20462   },
    {2,  0.6366564 },
  ...
  RegionResidualElement[] Lresiddata = new RegionResidualElement[] // Residual L for Thermal Conductivity
  {
    { 0, 0,  1.60397357000 },
    { 1, 0,  2.33771842000 },
    { 2, 0,  2.19650529000 },
  ...
  RegionIdealElement[] Lidealdata = new RegionIdealElement[] // Ideal L for thermal conductivity
  {
    {0,  2.443221E-3},
    {1,  1.323095E-2},
    {2,  6.770357E-3},
  ...
  static List<RegionResidualElement> Hrdata = new List<RegionResidualElement>(Hresiddata);
  static List<RegionIdealElement>    H0data = new List<RegionIdealElement>(   Hidealdata);
  static List<RegionResidualElement> Lrdata = new List<RegionResidualElement>(Lresiddata);
  static List<RegionIdealElement>    L0data = new List<RegionIdealElement>(   Lidealdata);
  ...
  class BaseRegion
  {
    public BaseRegion(List<RegionResidualElement> resid, List<RegionIdealElement> ideal)
    {
      ...
      {
        lamnr.Add(Lrdata[i].n);
        lamIr.Add(Lrdata[i].I);
        lamJr.Add(Lrdata[i].J);
      }
      for(int i = 0; i < L0data.size(); ++i)
      {
        lamn0.Add(L0data[i].n);
        lamJ0.Add(L0data[i].J);
      }
    }
  ...
  static List<RegionResidualElement> reg1rdata = new List<RegionResidualElement>(Region1residdata);
  static List<RegionIdealElement> reg10data;
  class Region1 : BaseRegion
  {
    Region1() : base(reg1rdata, reg10data)
    {
  ...
  static List<RegionResidualElement> reg2rdata = new List<RegionResidualElement>(Region2residdata);
  static List<RegionIdealElement> reg20data = new List<RegionIdealElement>(Region2idealdata);
  class Region2 : BaseRegion
  {
    Region2() : base(reg2rdata, reg20data)
    {
  ...
  static const List<double> region23_n = new List<double>(Region23data);
  ...
  static List<RegionResidualElement> reg3rdata = new List<RegionResidualElement>(Region3residdata);
  ...
  class Region3
  {
    ...
    Region3()
    {
      ...
      for(int i = 0; i < reg3rdata.size(); ++i)
      {
        nr.Add(reg3rdata[i].n);
        Ir.Add(reg3rdata[i].I);
        Jr.Add(reg3rdata[i].J);
      }
      for(int i = 0; i < Hrdata.size(); ++i)
      {
        munr.Add(Hrdata[i].n);
  ...
  static List<SaturationElement> reg4data = new List<SaturationElement>(sat);
  ...
  static List<RegionResidualElement> reg5rdata = new List<RegionResidualElement>(Region5residdata);
  static List<RegionIdealElement> reg50data = new List<RegionIdealElement>(Region5idealdata);
...
//     It may double as Tsat(h,s) in Region 4 as well.
    ...
    static const List<double> region2b2c_n = new List<double>(Region2b2cdata);

    double P2b2c_h(double h)
    {
    ...
    static const List<double> region3ab_n = new List<double>(Region3abdata);

    double H3ab_p(double p)
    {
      // Only called for Region determination and debugging.  No range checking.
      const double p_star = 1*p_fact;
      const double h_star = 1*R_fact;
    ...
    static const List<double> region2ab_n = new List<double>(Region2abdata);

    double H2ab_s(double s)
    {
      // Only called for Region determination and debugging.  No range checking.
  ...
  IF97REGIONS RegionDetermination_TP(double T, double p)
  {
    ...
    else if(T > T23min && T <= Tmax)
    {
      ...
      else if(p < 16.5292*p_fact) return REGION_2; // Check this one first to avoid the call to 2-3 boundary curve (a little bit faster)
      else if(p > Region23_T(T)) return REGION_3;
      else return REGION_2;
    }
    else if(T >= Tmin && T <= T23min)
    {
      if(p > Pmax) throw new ArgumentOutOfRangeException("Pressure out of range");
      else if(p > R4.p_T(T)) return REGION_1;
      else if(p < R4.p_T(T)) return REGION_2;
  ...
  double RegionOutput(IF97parameters outkey, double T, double p, IF97SatState State)
  {
    ...
    {
      case REGION_1:
        if(State == VAPOR) return R2.output(outkey, T, p);  // On saturation curve and need the Vapor phase
        else return R1.output(outkey, T, p);  // otherwise, use Liquid Region 1
        break;
      case REGION_2:
        if(State == LIQUID) return R1.output(outkey, T, p);  // On saturation curve and need the Liquid phase
      ...
      case REGION_3:
        ...
        break;
      case REGION_4:
        if(State == VAPOR) return R2.output(outkey, T, p);
        else if(State == LIQUID) return R1.output(outkey, T, p);
  ...
  IF97REGIONS RegionDetermination_pX(double p, double X, IF97parameters inkey)
  {
    ...
    {
      if(inkey == IF97_HMASS) throw new ArgumentOutOfRangeException("Enthalpy out of range");
      else                    throw new ArgumentOutOfRangeException("Entropy out of range");
    }
    ...
    if(p <= Pcrit)
    {
      ...
      if((Xliq <= X) && (X <= Xvap)) return REGION_4; // Within Saturation Dome //  Region 4
    }
    ...
    else if(X <= R1.output(inkey,T23min,p)) return REGION_1;
    else if(X >= R2.output(inkey,Region23_p(p),p)) return REGION_2;
    else return REGION_3;
  }  // Region Output backward


  int BackwardRegion(double p, double X, IF97parameters inkey)
  {
  ...
  double RegionOutputBackward(double p, double X, IF97parameters inkey)
  {
    ...
    {
      case REGION_1:
        if(inkey == IF97_HMASS) return B1H.T_pX(p,X);
  ...
  double rho_pX(double p, double X, IF97parameters inkey)
  {
    ...
    if(RegionDetermination_pX(p, X, inkey) == REGION_4)
  ...
  double Q_pX(double p, double X, IF97parameters inkey)
  {
    double Xliq;
    double Xvap;
  ...
  double X_pQ(IF97parameters inkey, double p, double Q)
  {
    double Xliq;
    double Xvap;
    if((p<Ptrip) || (p>Pcrit))
  ...
  static const List<double> Hmax_n = new List<double>(HTmaxdata);

  double Hmax(double s)
  {
  ...
  double Hmin(double s)
  {
    ...
    else return RegionOutput( IF97_HMASS, RegionOutputBackward(Pmin,s,IF97_SMASS), Pmin, NONE); // Use forward equation along Pmin using T(Pmin,s) as Temperature
  }

  IF97BACKREGIONS RegionDetermination_HS(double h, double s)
  {
    ...
    if(s <= SfT23)
    {
      if(h < Backwards.Hsat_s(s)) return BACK_4; // If below Saturated Liquid Curve // REGION 4
      else if(s < S13min)          return BACK_1; // If below H13 Curve              // REGION 1
      else
      { // IF within H13 Curve (S13min < s < SfT23)
        if(h < b13.h_s(s)) return BACK_1;  // below curve // REGION 1
        else               return BACK_3A; // above curve // REGION 3
      }
    }
    else if(s <= Scrit)
    { //========== Region 3a Check (S < Scrit) ==============
      if(h < Backwards.Hsat_s(s)) return BACK_4;  // If below Saturated Liquid Curve // REGION 4
      else                         return BACK_3A; // If above curve                  // REGION 3(a)
    }
    else if(s <= S23min)
    { //========== Region 3b Check      ==============
      if(h < Backwards.Hsat_s(s)) return BACK_4;  // If below Saturated Liquid Curve // REGION 4
      else                         return BACK_3B; // If above curve                  // REGION 3(a)
    }
    else if(s <= S23max)
    { //========== Region 3b/2c Check Along B23 Curve ======
      if(h < Backwards.Hsat_s(s)) return BACK_4;  // if below Saturated Vapor Curve // REGION 4
      else if(h < H23min)          return BACK_3B; // if below bounding box          // REGION 3(b)
      else if(h > H23max)          return BACK_2C; // if above bounding box          // REGION 2(c)
      else
      { //  Need to check TB23 Curve
        ...
        if(P > PB23) return BACK_3B;    // Above B23 Curve // REGION 3(b)
        else         return BACK_2C;    // Below B23 Curve // REGION 2(c)
      }
    }
    else if(s <= S2bc)
    { //========== Region 3b Check      ==============
      if(h < Backwards.Hsat_s(s)) return BACK_4;  // If below Saturated Liquid Curve // REGION 4
      else                         return BACK_2C; // If above curve                  // REGION 2(c)
    }
    else if(s < Sgtrip)
    { //========== Region 2a/2b (s > S2bc) above Sat. Curve ==
      if(h < Backwards.Hsat_s(s)) return BACK_4; // If below Saturated Vapor Curve // REGION_4
      else
      { //  If above Curve then
        if(h > Backwards.H2ab_s(s)) return BACK_2B; // if h > h2ab(s) Curve (P=4 MPa) // REGION 2(b)
        else                         return BACK_2A; // if h < h2ab(s) Curve (P=4 MPa) // REGION 2(a)
      }
    }
    else return BACK_2A; //========== Region 2a fall thru ========================
  } // Region Determination HS

  double BackwardOutputHS(IF97parameters outkey,double h, double s)
  {
    // Note that this routine returns only temperature (IF97_T).  All other values should be
    // calculated from this temperature and the known pressure using forward equations.
    // Setup Backward Regions for output
    ...
    if((outkey != IF97_P) && (outkey != IF97_T)) throw new ArgumentException("Backward HS Formulas output Temperature or Pressure only.");
    ...
    {
      if(region == BACK_4) return psat97(Tval); // Not REGION 4, already have pressure
      else return Pval;                         // REGION 4, Calculate Psat from Tsat
    }
    else               // ELSE Returning Temperature
    {
      if(region == BACK_4) return Tval;                     // REGION 4, already have Temperature
      else return RegionOutputBackward(Pval,h,IF97_HMASS);  // Not REGION 4 Calc from Backward T(p,h)
    }
  }  // Region Output backward
  ...
  double rhomass_Tp(double T, double p) { return RegionOutput( IF97_DMASS, T, p, NONE); }
  /// Get the mass enthalpy [J/kg] as a function of T [K] and p [Pa]
  double hmass_Tp(double T, double p) { return RegionOutput( IF97_HMASS, T, p, NONE); }
  /// Get the mass entropy [J/kg/K] as a function of T [K] and p [Pa]
  double smass_Tp(double T, double p) { return RegionOutput( IF97_SMASS, T, p, NONE); }
  /// Get the mass internal energy [J/kg] as a function of T [K] and p [Pa]
  double umass_Tp(double T, double p) { return RegionOutput( IF97_UMASS, T, p, NONE); }
  /// Get the mass constant-pressure specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cpmass_Tp(double T, double p) { return RegionOutput( IF97_CPMASS, T, p, NONE); }
  /// Get the mass constant-volume specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cvmass_Tp(double T, double p) { return RegionOutput( IF97_CVMASS, T, p, NONE); }
  /// Get the speed of sound [m/s] as a function of T [K] and p [Pa]
  double speed_sound_Tp(double T, double p) { return RegionOutput( IF97_W, T, p, NONE); }
  /// Get the [d(rho)/d(p)]T [kg/m�/Pa] as a function of T [K] and p [Pa]
  double drhodp_Tp(double T, double p) { return RegionOutput( IF97_DRHODP, T, p, NONE); }
  ...
  double visc_TRho(double T, double rho)
  {
  ...
  double visc_Tp(double T, double p) { return RegionOutput(IF97_MU, T, p, NONE); }
  /// Get the thermal conductivity [W/m-K] as a function of T [K] and p [Pa]
  double tcond_Tp(double T, double p) { return RegionOutput(IF97_K, T, p, NONE); }
  /// Calculate the Prandtl number [dimensionless] as a function of T [K] and p [Pa]
  double prandtl_Tp(double T, double p) { return visc_Tp(T,p) * cpmass_Tp(T,p) * (1000/R_fact) / tcond_Tp(T,p); }

  // ******************************************************************************** //
  //               Saturated Vapor/Liquid Functions           //
  // ******************************************************************************** //
  /// Get the saturated liquid mass density [kg/m^3] as a function of p [Pa]
  double rholiq_p(double p) { return RegionOutput( IF97_DMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass density [kg/m^3] as a function of p [Pa]
  double rhovap_p(double p) { return RegionOutput( IF97_DMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid mass enthalpy [J/kg] as a function of p [Pa]
  double hliq_p(double p) { return RegionOutput( IF97_HMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass enthalpy [J/kg] as a function of p [Pa]
  double hvap_p(double p) { return RegionOutput( IF97_HMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid mass entropy [J/kg/K] as a function of p [Pa]
  double sliq_p(double p) { return RegionOutput( IF97_SMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass entropy [J/kg/K] as a function of p [Pa]
  double svap_p(double p) { return RegionOutput( IF97_SMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid mass internal energy [J/kg] as a function of p [Pa]
  double uliq_p(double p) { return RegionOutput( IF97_UMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass internal energy [J/kg] as a function of p [Pa]
  double uvap_p(double p) { return RegionOutput( IF97_UMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid mass isobaric specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cpliq_p(double p) { return RegionOutput( IF97_CPMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass isobaric specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cpvap_p(double p) { return RegionOutput( IF97_CPMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid mass isochoric specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cvliq_p(double p) { return RegionOutput( IF97_CVMASS, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor mass isochoric specific heat [J/kg/K] as a function of T [K] and p [Pa]
  double cvvap_p(double p) { return RegionOutput( IF97_CVMASS, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid speed of sound [m/s] as a function of T [K] and p [Pa]
  double speed_soundliq_p(double p) { return RegionOutput( IF97_W, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor speed of sound [m/s] as a function of T [K] and p [Pa]
  double speed_soundvap_p(double p) { return RegionOutput( IF97_W, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid viscosity [Pa-s] as a function of p [Pa]
  double viscliq_p( double p) { return RegionOutput( IF97_MU, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor viscosity [Pa-s] as a function of p [Pa]
  double viscvap_p( double p) { return RegionOutput( IF97_MU, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Get the saturated liquid thermal conductivity [W/m-K] as a function of p [Pa]
  double tcondliq_p( double p) { return RegionOutput( IF97_K, Tsat97(p), p, LIQUID); }
  /// Get the saturated vapor thermal conductivity [W/m-K] as a function of p [Pa]
  double tcondvap_p( double p) { return RegionOutput( IF97_K, Tsat97(p), p, VAPOR); }
  // ******************************************************************************** //
  /// Calculate the saturated liquid Prandtl number [dimensionless] as a function of p [Pa]
  double prandtlliq_p(double p) { return viscliq_p(p) * cpliq_p(p) * (1000/R_fact) / tcondliq_p(p); }
  /// Calculate the saturated vapor Prandtl number [dimensionless] as a function of p [Pa]
  double prandtlvap_p(double p) { return viscvap_p(p) * cpvap_p(p) * (1000/R_fact) / tcondvap_p(p); }
  ...
  //                Backward Functions                  //
  // ******************************************************************************** //
  double T_phmass(double p,double h) { return RegionOutputBackward( p, h, IF97_HMASS); }
  double rhomass_phmass(double p,double h) { return rho_pX( p, h, IF97_HMASS); }
  double T_psmass(double p,double s) { return RegionOutputBackward( p, s, IF97_SMASS); }
  double rhomass_psmass(double p,double s) { return rho_pX( p, s, IF97_SMASS); }
  double p_hsmass(double h, double s) { return BackwardOutputHS(IF97_P, h, s); }
  double T_hsmass(double h, double s) { return BackwardOutputHS(IF97_T, h, s); }
  int Region_ph(double p, double h) { return BackwardRegion( p, h, IF97_HMASS); }
  int Region_ps(double p, double s) { return BackwardRegion( p, s, IF97_SMASS); }
  // ******************************************************************************** //
  //                Trivial Functions                   //
  // ******************************************************************************** //
  /// Get the Triple Point Temperature and Pressure
  double get_Ttrip() { return Ttrip; }
  double get_ptrip() { return Ptrip; }
  /// Get the Critical Point Temperature and Pressure and Density
  double get_Tcrit() { return Tcrit; }
  double get_pcrit() { return Pcrit; }
  double get_rhocrit() { return Rhocrit; }
  /// Get the Max and Min Temperatures and Pressures
  double get_Tmin() { return Tmin; }
  double get_Pmin() { return Pmin; }
  double get_Tmax() { return Tmax; }
  double get_Pmax() { return Pmax; }
  /// Get physical constants
  double get_MW() { return MW; }
  double get_Rgas() { return Rgas; }
  double get_Acentric() { return -log10(psat97(0.7*Tcrit)/Pcrit) - 1; }
  // ******************************************************************************** //
  //                Utility Functions                   //
  // ******************************************************************************** //
  ...
  double hmass_pQ(double p,double Q) { return X_pQ(IF97_HMASS, p, Q); }
  double umass_pQ(double p,double Q) { return X_pQ(IF97_UMASS, p, Q); }
  double smass_pQ(double p,double Q) { return X_pQ(IF97_SMASS, p, Q); }
  double v_pQ(double p,double Q) { return 1.0/X_pQ(IF97_DMASS, p, Q); }
  double rhomass_pQ(double p,double Q) { return X_pQ(IF97_DMASS, p, Q); }
  double Q_phmass(double p,double h) { return Q_pX(p, h, IF97_HMASS); }
  double Q_pumass(double p,double u) { return Q_pX(p, u, IF97_UMASS); }
  double Q_psmass(double p,double s) { return Q_pX(p, s, IF97_SMASS); }
  double Q_prhomass(double p,double rho) { return Q_pX(p, rho, IF97_DMASS); }
  double Q_pv(double p,double v) { return Q_pX(p, 1.0/v, IF97_DMASS); }
/*************************************************************************/
...
static List<Table3Data> Table3 = new List<Table3Data>(_Table3);
